...
function saveCarStateToLocalStorageForGoogleSheets(carState) {
  const key = document.title
...
function getCarStateFromLocalStorageForGoogleSheets() {
  ...
  if (!log) {
    gExel()
    console.log('записи нет', log)
  } else {
    carState = savedState.carState
    console.log('записm есть', log)
    loadConfigutation()
  }
}
    ...
    for (let i = 0; i < data.length; i++) {
      if (data[i]) {
        ...
        if (obj.show) {
          ...
          console.log('show true', processedData)
        } else {
          console.log('show false', obj)
        }
      }
    }
