...
const removeShowDopOptions = () => {
  ...
  carState.options.runningBoards[0].forEach((option, index) => {
    ...
    const modelsToShow = option.models
    const showOption =
...
const createDivPriceModel = (modelName) => {
  ...
  divPriceModel.className = `car${modelName}Model`
...
const tableBody4 = data4.map((item) => [
  { text: item, alignment: 'left', fontSize: 10, margin: [0, 0, 0, 8] },
])
const myPDF = async () => {
  ...
  const data2 = [
    saleAll && saleAll !== 0
      ? { label: 'СУММАРНАЯ СКИДКА:', value: `${saleAll} руб.` }
      : '',
    {
      label:
        saleAll && saleAll !== 0
    ...
    },
    {
      ...
      value: `${numberWithSpaces(
        calculateWithPercentage(totalCarPrice, carState.marga[1]) + 4e4
      )} руб.`,
    },
    {
      label: 'СТОИМОСТЬ Б/Н С НДС 20%:',
      value: `${numberWithSpaces(
        calculateWithPercentage(totalCarPrice, carState.marga[2]) + 4e4
      )} руб.`,
    },
  ...
  const tableBody5 = await Promise.all(
    data5.map(async ({ img, contact }) => {
      ...
      return [
        { image: i, width: 24, height: 24, margin: [0, 0, 0, 16] },
        {
          text: contact,
          alignment: 'left',
          fontSize: 12,
  ...
  var pdfContent = {
    ...
    content: [
      {
        canvas: [
          { type: 'rect', x: -16, y: 0, w: 563, h: 0.5, color: '#000000' },
        ],
      },
      {
      ...
      },
      {
        columns: [
          {
            margin: [0, 0, 0, 0],
            table: { widths: [120, 120], body: tableBody },
          },
          {
            margin: [0, 0, 0, 0],
            table: {
              ...
              body: [
                ...
                ...tableBody2,
                [
                  { text: '', border: [0, 0, 0, 0], margin: [0, 26, 0, 0] },
                  {
      ...
      },
      {
        columns: [
          [
            { image: image1, width: 24, height: 24, margin: [0, 0, 0, 16] },
            {
          ...
          [
            { image: image2, width: 24, height: 24, margin: [0, 0, 0, 16] },
            {
          ...
          [
            { image: image3, width: 24, height: 24, margin: [0, 0, 0, 16] },
            {
          ...
          [
            { image: image4, width: 24, height: 24, margin: [0, 0, 0, 16] },
            {
    ...
    styles: { header: { fontSize: 14, color: '#222222', bold: false } },
  }
